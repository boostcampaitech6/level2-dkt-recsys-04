...
class Preprocess:
    ...
    # feat siyun : is_train의 처리를 None으로 변경
    def load_data_from_file(self, file_name: str, is_train: bool = True) -> np.ndarray:
        ...
        self.user2index = {user_id: idx for idx, user_id in enumerate(df['userID'].unique())}
        self.item2index = {item_id: idx for idx, item_id in enumerate(df['assessmentItemID'].unique())}

        
        # feat siyun : 확인용
        # print('user2index, item2index 확인용')
        # print(len(self.user2index))
        # print(len(self.item2index))
        # train 일 때 
        # 6698,9454
        # test 일때
        # 744,9454임을 확인
    ...
    def load_train_data(self, file_name: str) -> None:
        train_data = self.load_data_from_file(file_name) 
        self.train_data = train_data 
        
    # (csv data 로드 + feature engineering + 정리)한 것을 none이었던 self.test_data에 정의
    def load_test_data(self, file_name: str) -> None:
        test_data = self.load_data_from_file(file_name)
        self.test_data = test_data


class DKTDataset(torch.utils.data.Dataset):
    # def __init__(self, data: np.ndarray, args, user_test_indices=None, item_test_indices=None, is_train=None):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        ## feat siyun
        data['user_idx'] = torch.tensor(row[-2], dtype=torch.int)  # row[-2]는 user_idx
        data['item_idx'] = torch.tensor(row[-1], dtype=torch.int)  # row[-1]는 item_idx
...
# feat siyun: add is_train, user/item test indices
def get_loaders(args, train, valid=None, is_train=None, user_test_indices=None, item_test_indices=None, preprocess=Preprocess) -> Tuple[torch.utils.data.DataLoader]:
    pin_memory = False
    ...
    if train is not None:
        ...
        # feat siyun : add indices, is_train
    if valid is not None:
        # valset = DKTDataset(valid, args, is_train=is_train,user_test_indices=user_test_indices, item_test_indices=item_test_indices)
