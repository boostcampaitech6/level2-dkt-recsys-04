...
def run(args,
    ...
    # feat siyun : add preprocess
    train_loader, valid_loader = get_loaders(args=args, train=train_data, valid=valid_data, preprocess=preprocess)
...
def inference(args, test_data: np.ndarray, model: nn.Module) -> None:
    ...
    for step, batch in enumerate(test_loader):
        ...
        preds = model(batch) # [건우] '**'를 사용하기 위해 parameter와 argument의 쌍이 같아햐 하는데 lstm에서 paramete는 data하나기 때문에 '**'안씀
        #feat siyun
        # preds = model(batch, user_test_emb=preprocess.user_test_emb, item_test_emb=preprocess.item_test_emb)
...
def get_model(args,user2index, item2index) -> nn.Module:
    ...
    try:
        ...
        model = { # get은 사용자 입력
            ...
            "lastquery": LastQuery,
            "saint": Saint,
            # feat siyun : add indexes
        }.get(model_name)(args,user2index, item2index) # [건우] model.py에서 각 모델의 init으로 args만 받기 때문
    except KeyError:
...
# feat siyun : add user/item2index
## is_train의 여부에 따라 달라지기 때문에 test에 한정하는 변수를 만들 필요가 없음.
## None을 해주는 이유 : 인자의 선택을 optional하게 만들어야 유연성이 생김.
def load_model(args, get_model, user2index, item2index):
    ...
    # 2.feat siyun : add user_emb, item_emb
    # if user2index is not None and item2index is not None:
    #     model.user_emb = user2index
    #     model.item_emb = item2index
